...
## 新しいドリルを追加するときのチェックリスト

ドリルを1つ追加するたびに、以下をセットで実装する。

### 1. ホーム表示 ON/OFF

| ファイル | 追加内容 |
|----------|----------|
| `storage_service_web.dart` | `const _show{Drill}Key = 'show_{drill_key}';` + `loadShow{Drill}()` / `saveShow{Drill}()` |
| `storage_service_stub.dart` | 同シグネチャのスタブ（デフォルト値はメインドリルなら `true`、オプションなら `false`） |
| `home_visibility_service.dart` | `static final show{Drill}Notifier = ValueNotifier<bool>(StorageService.loadShow{Drill}());` + `setShow{Drill}(bool)` |
| `home_screen.dart` | ボタンを `ValueListenableBuilder<bool>(show{Drill}Notifier, ...)` でラップ |
| `settings_screen.dart` | 「ホーム画面のボタン」セクションに `Switch` トグルを追加 |

**既存の実装例:** `show_clock` → `HomeVisibilityService.showClockNotifier`

### 2. 難易度設定（難易度の概念があるドリルのみ）

| ファイル | 追加内容 |
|----------|----------|
| `storage_service_web.dart` | `const _{drill}DiffKey = '{drill}_difficulty';` + `load{Drill}Difficulty()` / `save{Drill}Difficulty()` |
| `storage_service_stub.dart` | 同スタブ（デフォルトは最もやさしい値） |
| `settings_screen.dart` | 「かんじのむずかしさ」と同パターンで選択ボタン列 `_DifficultyButton` を追加 |
| 各ドリル画面 `initState` | `StorageService.load{Drill}Difficulty()` でプールをフィルタ |

**既存の実装例:** `kanji_max_strokes` → `_KanjiLevelButton`（settings_screen.dart 末尾）、かんじ画面の `_pool` フィルタ

## バトル・スゴロクのドリル設定継承

バトル（`battle_screen.dart`）・スゴロク（`sugoroku_screen.dart`）は「その問題ジャンルが個別ドリルで有効かつ、同じ難易度設定」で出題する。

### 原則

- 問題ジャンルをバトル/スゴロクに追加する際は、対応ドリルの設定をそのまま読む
  - 例: かんじ問題を追加する → `StorageService.loadKanjiMaxStrokes()` で難易度を決定
  - 例: かんじをバトルに出さない設定 → `StorageService.loadShowKanjiInBattle()` (bool) で制御
- ジャンル有効/無効のキーは `show_{drill_key}_in_battle` 形式（デフォルト: true）
- `_generateQuiz(zone)` の冒頭で有効ジャンルリストを組み立ててから `type` をランダム選択する

### バトルにジャンルを追加するときの実装パターン

1. `StorageService` に `loadShow{Drill}InBattle()` / `saveShow{Drill}InBattle()` を追加
2. 設定画面「バトル設定」セクションにトグルを追加
3. `battle_screen.dart` の `initState` でフラグを読んで `_enabledTypes` を構築
4. `_generateQuiz` は `_enabledTypes` からジャンルを選ぶ
5. 難易度は対応ドリルの設定キーを直接 `StorageService.load{Drill}Difficulty()` で読む
